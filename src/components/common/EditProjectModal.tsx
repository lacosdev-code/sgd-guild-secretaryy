...
const STATUS_OPTIONS = ['Active', 'Completed', 'On Hold', 'At Risk', 'Archived']

export default function EditProjectModal({ project, arcs, onClose }: EditProjectModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!name.trim()) return notify.warn('Nama Project tidak boleh kosong.')
    setLoading(true)
    ...
    try {
      const res = await fetch(`/api/projects/${project.id}`, {
        ...
        body: JSON.stringify({ name: name.trim(), arcId: arcId || null, status })
      })
      if (!res.ok) throw new Error(await res.text())

      notify.success('Project berhasil diperbarui!')
      router.refresh()
      onClose()
    } catch (err: any) {
      notify.error('Gagal memperbarui Project: ' + err.message)
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={onClose}>
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-bold tracking-widest uppercase mb-1.5 text-gray-500 dark:text-[#C9A227]/70">Nama Project *</label>
            <input
              type="text"
              value={name}
          ...
          <div>
            <label className="block text-xs font-bold tracking-widest uppercase mb-1.5 text-gray-500 dark:text-[#C9A227]/70">Arc (Opsional)</label>
            <select
              value={arcId}
